...
export default function Home({ params }: { params: { name: string } }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const targetCard = fieldsToSave.find(
        (card) => card.cardTitle === "Data" && card.cardName === "AWS"
      );
      
      if (targetCard) {
        const targetField = targetCard.fieldsValue.find(
          (field) => field.fieldName === "AWS BUCKET NAME"
        );
      
        if (targetField) {
          console.log("AWS BUCKET NAME Value:", targetField.value);
        } else {
          console.log("AWS BUCKET NAME field not found.");
        }
      } else {
        console.log("Card with title 'Data' and name 'AWS' not found.");
      }
  ...
  return (
    <div className="min-h-screen  w-screen dark-shadow ">
      <SidebarProvider>
        <AppSidebar />
        <main>
          <SidebarTrigger />
         <main className="flex-1 bg-black/30 w-[96vw] rounded-br-3xl rounded-bl-3xl border-white border-b-2 p-10">
            ...
            {card.map((card) => (
                <div key={card.id} className="border p-4 shadow-gray-700 text-white sched3  relative rounded-lg shadow-lg">
                   <button
  onClick={() => {
    if (card.id) {
      removeCard(card.id);
...
>
  <IoMdClose size={20} />
                  ...
                  <h3 className="text-base mt-2 text-gray-400 font-bold">{card.name}</h3>
    ...
    cardData.map((card, cardIndex) => (
      <BackdropGradient
        ...
        >
      <div key={cardIndex} className="border p-4 bg-grid-small-white/[0.3] border-white/40 bg-black text-white shadow-gray-700 rounded-lg shadow-lg">
        {/* Display Title and Name */}
                    <div className="flex justify-between">
              <div>
                <h3 className="text-2xl mb-2 font-bold">{card.cardTitle}</h3>
                <h3 className="text-base text-gray-300 font-medium">{card.cardName}</h3>
              </div>
                  ...
                  <Input
                    ...
                    defaultValue={
                      field.value.length > 5
                        ? `${field.value.slice(0, 3)}${"*".repeat(field.value.length - 5)}${field.value.slice(-2)}`
                        : field.value // Show full value if it's too short
                    }
                                   />
                ) : (
                  <Input
                    ...
                    className="ml-2 p-1 border bg-black pointer-events-none rounded-md w-full"
                    defaultValue={
                      field.fieldName === "AWS BUCKET NAME"
                        ? field.value // Show full value for AWS BUCKET NAME
                        : field.value.length > 5
                        ? `${field.value.slice(0, 3)}${"*".repeat(field.value.length - 5)}${field.value.slice(-2)}`
                        : field.value // Show full value if too short
                    }        />
                )}
              </div>
            </li>
